...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    // 手动 LipSync 覆盖（外部控制用，确保在 onLateUpdate 之后生效）
    if (this._lipSyncForm !== null) {
      const idMgr = CubismFramework.getIdManager();
      this._model.setParameterValueById(idMgr.getId('ParamMouthForm'), this._lipSyncForm);
      this._model.setParameterValueById(idMgr.getId('ParamMouthOpenY'), this._lipSyncOpen);
    }
   ...
   * 自动待机动作のON/OFFを设定
   */
  public setAutoIdle(enabled: boolean): void {
    this._autoIdle = enabled;
  }

  /**
   * 自动待机动作の状态を取得
   */
  public getAutoIdle(): boolean {
    return this._autoIdle;
  }

  /**
   * 启动手动 LipSync（每帧强制覆盖嘴参数）
   */
  public startManualLipSync(form: number, open: number): void {
    this._lipSyncForm = form;
    this._lipSyncOpen = open;
  }

  /**
   * 停止手动 LipSync（同时复位嘴参数到自然值，防止 saveParameters 固化）
   */
  public stopManualLipSync(): void {
    this._lipSyncForm = null;
    this._lipSyncOpen = null;
    // 显式复位嘴参数到自然闭嘴状态
    const idMgr = CubismFramework.getIdManager();
    this._model.setParameterValueById(idMgr.getId('ParamMouthForm'), 1.0);
    this._model.setParameterValueById(idMgr.getId('ParamMouthOpenY'), 0.0);
  }
  ...
  public constructor() {
    ...
    this._autoIdle = false; // 【关键改动】false = 不自动循环待机，保持初始姿态一致；true = 自动随机 idle
  ...
  private _autoIdle: boolean; // 自動待機モーションON/OFF
  private _lipSyncForm: number | null = null; // 手动 LipSync 口形
  private _lipSyncOpen: number | null = null; // 手动 LipSync 开度
