...
**修复**：在 `Live2DPage.javaScriptConsoleMessage` 中添加关键词过滤。
...
**根因**：
SDK 在 `lappdelegate.ts` 里把 `pointerdown/pointerup` 挂到了 `document` 上，所有页面点击都会进入 `lappsubdelegate.ts` 的事件处理。但坐标转换用的是 `canvas.offsetLeft/offsetTop`，这在 canvas 被 MutationObserver 动态迁移到 `#live2d-panel`（flex 右侧布局）后不准确——`offsetLeft/offsetTop` 相对于父元素，而非文档整体。

**最终方案**（`lappsubdelegate.ts`）：
1. 统一用 `canvas.getBoundingClientRect()` 替代 `offsetLeft/offsetTop`
2. `onPointBegan`：先算坐标再判断是否在 canvas 内，只有在范围内才设 `_captured = true`
3. `onPointMoved` / `onPointEnded` / `onTouchCancel`：先检查 `_captured`
4. 编译后需同步 `dist/assets/*` → `web/sdk_dist/assets/`，更新 `index.html` 引用

## 13. tsconfig.json baseUrl 弃用警告（可忽略）
TS 5.9 不支持 `ignoreDeprecations`。`npm run build` 完全通过，仅 IDE schema 提示。

## 14. TTS 语音合成部署错误集（2026-06-28）
...
## 15. 背景图不生效 / CSS 改 background 无效（2026-07-14）
...
## 16. Python 3.9 不支持 `X | None` 联合类型简写（2026-08-04）

**现象**：`TypeError: unsupported operand type(s) for |: 'type' and 'NoneType'`

```python
_client: OpenAI | None = None   # ❌ Python 3.9 报错
```

**根因**：`X | None` 语法是 Python 3.10 引入的（PEP 604）。项目 venv 使用 Python 3.9.5。

**修复**：改用 `typing.Optional`（Python 3.6+ 兼容）：
...
> 同时注意 `pip.exe` 中硬编码了旧路径（项目迁移导致），安装新包需用 `python -m pip install` 绕过。
